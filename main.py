...
from core.utils import load_signals_db, load_progress
from core.sounds import play_ambience


# ----------------------------------------------------------------------
#  Вспомогательные функции для чтения клавиш (без Enter)
# ----------------------------------------------------------------------

def _getch():
    """Читает один символ из stdin без ожидания Enter (неблокирующий)."""
    fd = sys.stdin.fileno()
    old_settings = termios.tcgetattr(fd)
    try:
        tty.setraw(sys.stdin.fileno())
        ch = sys.stdin.read(1)
        if ch == '\x1b':  # escape-последовательность (стрелки)
...
def _kbhit():
    """Проверяет, есть ли данные в stdin (неблокирующий)."""
    fd = sys.stdin.fileno()
    ...
    try:
        ...
        ch = sys.stdin.read(1)
        if ch:
            # если есть символ, возвращаем его обратно (имитация peek)
            sys.stdin.seek(0, os.SEEK_CUR)  # это не совсем корректно, но для простоты
            return True
        return False
    finally:
...
def get_key():
    """Возвращает считанную клавишу (UP, DOWN, ENTER, или символ)."""
    ch = _getch()
    if ch == '\x1b[A':
        return "UP"
    elif ch == '\x1b[B':
...
def clear_screen():
    """Очищает экран и перемещает курсор в начало."""
    sys.stdout.write("\033[2J\033[H")
    sys.stdout.flush()


def print_centered(text, width=70):
    """Печатает текст по центру заданной ширины."""
    sys.stdout.write(text.center(width) + "\n")


def show_menu():
    """Отображает главное меню и возвращает выбранный пункт (0,1,2)."""
    options = ["Начать игру", "Загрузить игру", "Выйти"]
    selected = 0
    ...
    while True:
        ...
        # Заголовок
        print("\033[1;36m")  # яркий циан
        print_centered("╔══════════════════════════════════════════════════════╗")
        print_centered("║                                                      ║")
        print_centered("║                  V  O  I  D  M  A  P                 ║")
        print_centered("║                                                      ║")
        print_centered("╚══════════════════════════════════════════════════════╝")
        print("\033[0m")  # сброс
        ...
        for i, opt in enumerate(options):
            if i == selected:
                ...
                print(f"    {opt}")
        print("\n  \033[2;37m↑/↓  —  выбор   |   Enter  —  подтвердить\033[0m")
...
# ----------------------------------------------------------------------
#  Анимация загрузки (boot-последовательность)
# ----------------------------------------------------------------------

def boot_sequence():
    """Имитирует загрузку системы."""
    print("\033[2J\033[H")
    lines = [
...
"[   12.233234] THA-1500 system is initialized successfully",
"[   13.023474] UI is initialized",
    ]
    for line in lines:
        print(line)
        time.sleep(0.02)
...
def login_prompt():
    """Запрашивает имя пользователя и пароль. Возвращает имя."""
    print("\033[1;33m")
    print("    Добро пожаловать в систему управления телескопом ТНА-1500")
    print("    -------------------------------------------------------")
    print("\033[0m")
...
def main():
    # Фоновая атмосфера (если есть файл)
    ...
    # Загрузка базы сигналов (всегда)
    all_signals = load_signals_db()

    if choice == 0:  # Начать игру
        boot_sequence()
        username = login_prompt()
        # Создаём новый мир с пустыми источниками, но с базой
        world = World()
        world.all_signals = all_signals
        world.sources = []          # пока нет сохранённых
        world.processed_signal_names = set()
        world.alien_reputation = 0
        world.alien_contact_stage = 0
        ...
        if not os.path.exists("savegame.json"):
            print("\033[1;31m[ОШИБКА] Файл сохранения не найден.\033[0m")
            time.sleep(2)
            main()  # возврат в меню (рекурсивно, но можно просто перезапустить)
            return
        boot_sequence()
        # Загружаем прогресс
        world = World()
        world.all_signals = all_signals
        load_progress(world, "savegame.json")   # предполагаем, что эта функция заполняет world.sources и т.д.
        # Если в сохранении нет имени пользователя, запросим
        if hasattr(world, 'username') and world.username:
    ...
    # Можно переопределить intro, чтобы добавить имя пользователя
    console.intro = (
        f"\nДобро пожаловать, {world.username}.\n"
        "Радиотелескоп ТНА-1500 готов к работе.\n"
